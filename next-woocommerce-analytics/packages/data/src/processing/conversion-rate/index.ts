...
type RawConversionRateReportDataItem =
	ReportsConversionRateByDateResponse[ 'items' ][ number ];
...
function sanitizeConversionRateItem(
	...
	// Calculate conversion rate as completed checkouts / active sessions
	const conversionRate =
		activeSessionsNum > 0
			? ( completedCheckoutNum / activeSessionsNum ) * 100
			: 0;
...
export const sanitizeReportConversionRateResponse = (
	...
	const steps: FunnelStep[] = [
		{
			id: 'sessions',
			label: __( 'Sessions', 'woocommerce-analytics' ),
			count: sanitizedSummary.active_sessions,
			rate: 100, // Starting point
		},
		{
			id: 'cart-addition',
			label: __( 'Cart', 'woocommerce-analytics' ),
		...
		{
			id: 'checkout',
			label: __( 'Checkout', 'woocommerce-analytics' ),
		...
		{
			id: 'completed',
			label: __( 'Purchase', 'woocommerce-analytics' ),
	...
	return {
		...
		data: response?.items
			? response.items.map( sanitizeConversionRateItem )
			: [],
		steps,
