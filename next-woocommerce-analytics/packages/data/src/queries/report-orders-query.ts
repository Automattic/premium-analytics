...
const getReportOrdersQueryKey = (
	p: RequestReportOrdersParams,
	hasProductFilters: boolean
) =>
	[
		'reports',
		'orders',
		hasProductFilters ? 'by-product-type' : 'by-date',
		p.from,
		p.to,
		p.interval,
		p.filters,
...
export function reportOrdersQuery(
	...
	return {
		queryKey: getReportOrdersQueryKey( params, hasProductFilters ),
		queryFn: async () => {
