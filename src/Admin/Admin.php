...
class Admin implements RegistrableInterface {
	...
	public function register(): void {
		 ...
		 * Load the generated backend entry point
		 * only if the Next Admin plugin is active.
		 */
		if (
			is_plugin_active( 'next-admin/next-admin.php' ) &&
			file_exists( WC_ANALYTICS_NEXT_ADMIN_ABSPATH . 'build/index.php' )
		) {
			require_once WC_ANALYTICS_NEXT_ADMIN_ABSPATH . 'build/index.php';
			...
			add_action(
				...
				function () {
					...
					register_setting(
						'general',
						'wc_analytics_site_status_by_order_product_type',
						array(
							'type'         => 'string',
							'show_in_rest' => true,
						)
			...
			add_filter( 'get_user_metadata', array( $this, 'populate_site_status_modal' ), 99, 4 );
			add_action( 'next_admin_init', array( $this, 'analytics_register_next_admin_menu_items' ), 20 );
			add_action( 'next_admin_init', array( $this, 'hook_next_admin_init' ), 20 );
			add_filter( 'next_admin_dashboard_widgets', array( $this, 'add_analytics_widgets_to_core_dashboard' ), 20 );
			add_action( 'next_admin_legacy_menu_items_exclude', array( $this, 'analytics_next_exclude_legacy_menu_items' ), 20 );
		}
	}
	...
	public function analytics_add_mvp_menu_items(): void {
		...
		wc_admin_register_page(
			array(
				...
				'title'    => __( 'Analytics (Beta)', 'woocommerce-analytics' ),
	...
	/**
	 * Register the new admin pages for the new Admin Layout (Beta).
	 *
	 * @return void
	 */
	public function analytics_register_new_admin_pages(): void {
		add_menu_page(
			__( 'Analytics (Beta)', 'woocommerce-analytics' ),
			__( 'Analytics (Beta)', 'woocommerce-analytics' ),
	...
	/**
	 * Excludes legacy menu items from the Next Admin interface.
	 *
	 * This function adds specific menu items to the list of legacy menu items that should
	 * be excluded from the Next Admin interface. It ensures that Analytics standalone
	 * menu items are not displayed in the Next Admin interface.
	 *
	 * @param array $legacy_menu_items The list of legacy menu items.
	 * @return array The list of legacy menu items.
	 */
	public function analytics_next_exclude_legacy_menu_items( $legacy_menu_items ) {
		return array_merge(
			$legacy_menu_items,
			array(
				'wc-analytics',
				'wc-admin&path=/woocommerce-analytics/settings',
			)
		);
	}
	...
	public function is_new_admin_layout_page( $hook ) {
		$new_admin_pages = array(
			...
			'analytics-beta_page_wc-analytics/dashboard',

			'analytics-beta_page_wc-analytics/reports',
			...
			'analytics-beta_page_wc-analytics/settings',
		);
	...
	/**
	 * Register the menu items for the Next Admin plugin.
	 *
	 * @return void
	 */
	public function analytics_register_next_admin_menu_items(): void {
		if ( ! function_exists( 'next_admin_register_admin_route' ) ) {
			return;
		}

		// Root menu item.
		next_admin_register_menu_item(
			'wc-analytics',
			array(
				'label'       => __( 'Analytics', 'woocommerce-analytics' ),
				'icon'        => 'dashicons-chart-bar',
		...
		next_admin_register_menu_item(
			...
			array(
				...
				'label'  => __( 'Dashboard', 'woocommerce-analytics' ),
				'icon'   => 'dashicons-chart-bar',
				'parent' => 'wc-analytics',
			)
		);
		...
		next_admin_register_menu_item(
			'wc-analytics-reports',
			array(
				'to'     => '/wc-analytics/reports',
				'label'  => __( 'Reports', 'woocommerce-analytics' ),
				'icon'   => 'dashicons-analytics',
				'parent' => 'wc-analytics',
			)
		);

		// Add dev menu only if the feature flag is enabled.
		if ( Features::is_enabled( 'addDevMenu' ) ) {
			next_admin_register_menu_item(
				'wc-analytics-dev',
				array(
					'to'     => '/wc-analytics/dev-tools',
					'label'  => __( 'Dev tools', 'woocommerce-analytics' ),
	...
	/**
	 * Hook into NextAdmin's initialization process.
	 *
	 * Registers callbacks to be executed during NextAdmin initialization,
	 * ensuring WooCommerce Analytics boot script is enqueued only when
	 * NextAdmin is active and properly initialized.
	 *
	 * @return void
	 */
	public function hook_next_admin_init(): void {
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_next_admin_boot' ), 1 );
	}

	/**
	 * Enqueue the NextAdmin boot script module.
	 *
	 * @return void
	 */
	public function enqueue_next_admin_boot(): void {
		$asset_path = plugin_dir_path( WC_ANALYTICS_NEXT_ADMIN_ABSPATH . '/woocommerce-analytics.php' ) . 'build/boot/index.asset.php';
		$src        = plugins_url( 'build/boot/index.js', WC_ANALYTICS_NEXT_ADMIN_ABSPATH . '/woocommerce-analytics.php' );
		$asset      = file_exists( $asset_path ) ? include $asset_path : array();

		$handle = $asset['name'] ?? 'woocommerce-analytics-boot';

		// Register the script module if it doesn't exist.
		wp_register_script_module(
			$handle,
			$src,
			$asset['moduleDependencies'] ?? array(),
			$asset['version'] ?? null
		);
	...
	/**
	 * Add Analytics widgets to the core dashboard.
	 *
	 * @param array $widgets The widgets to add.
	 * @return array The widgets.
	 */
	public function add_analytics_widgets_to_core_dashboard( $widgets ): array {
		$uids = array_column( $widgets, 'uid' );
		if ( in_array( '4', $uids ) ) {
			return $widgets;
		}
		...
		$widgets[] = array(
			'uid'        => '4',
			'type'       => 'woocommerce-analytics/at-a-glance',
			'attributes' => array(
				'from'           => null,
				'to'             => null,
				'preset'         => 'last-30-days',
	...
	/**
	 * Populate site status modal state in user preferences
	 *
	 * Extends user preferences with siteStatusShowModal based on site status.
	 * The modal is auto-dismissed (true) for non-mixed sites, and shown (false) for mixed sites.
	 *
	 * @param mixed  $pre      The value to return.
	 * @param int    $user_id  User ID.
	 * @param string $meta_key Meta key.
	 * @param bool   $single   Whether to return a single value.
	 * @return mixed           The extended preferences with modal state.
	 */
	public function populate_site_status_modal( $pre, $user_id, $meta_key, $single ) {
		global $wpdb;
		...
		remove_filter( 'get_user_metadata', array( $this, 'populate_site_status_modal' ), 99 );
		...
		if ( ! isset( $base['woocommerce/woocommerce-analytics']['siteStatusShowModal'] ) ) {
			// The modal should be shown only if site status is 'mixed'.
			$base['woocommerce/woocommerce-analytics']['siteStatusShowModal'] = OrderProductTypeTracker::is_mixed_statuses();
		}

		return $single ? array( $base ) : $base;
	}
}
