 ...
 * @internal
 */
class SalesByCampaignController extends AbstractCSVReportController {
	...
	public function get_report_label(): string {
		return __( 'Sales by Campaign', 'woocommerce-analytics' );
	}
	...
	public function get_column_headers( ?string $interval = null ): array {
		return array(
			'campaign'            => __( 'Campaign', 'woocommerce-analytics' ),
	...
	public function format_row_for_csv( array $item ): array {
		// Skip rows where both campaign and label are empty.
		if ( empty( $item['campaign'] ) && empty( $item['label'] ) ) {
			return array();
		}
