...
final class Plugin implements RegistrableInterface {
	...
	public function __construct( ContainerInterface $container ) {
		...
		$this->declare_wp_consent_api_compatibility();
	}
	...
	private function init_hooks() {
		...
		// Only load the module if WC_ANALYTICS is not already defined to avoid conflicts when the feature is migrated to WooCommerce core.
		if ( ! defined( 'WC_ANALYTICS' ) ) {
			// Define WC_ANALYTICS to stop Jetpack from initializing its tracking module.
			define( 'WC_ANALYTICS', true );

			add_action(
				'after_setup_theme',
				array( $this, 'load_woocommerce_analytics_tracking_module' )
			);
		}
	}

	/**
	 * Get the ApiProxy instance from the DI container.
	 *
	 * @since x.x.x
	 *
	 * @return ApiProxy
	 */
	public function get_api_proxy(): ApiProxy {
		return $this->container->get( ApiProxy::class );
	}
	...
	public function activate(): void {
		...
		if ( method_exists( Woocommerce_Analytics::class, 'maybe_add_proxy_speed_module' ) ) {
			Woocommerce_Analytics::maybe_add_proxy_speed_module();
		}

		// Initialize the order product type scan on plugin activation.
		// This ensures that any existing orders are scanned for product types.
		try {
			$order_tracker = $this->container->get( \Automattic\WooCommerce\Analytics\Utilities\OrderProductTypeTracker::class );
			$order_tracker->register(); // Ensure hooks are registered. This would be run later during normal execution as well, but doens't hurt to call it again.
			$order_tracker->maybe_initialize_scan();
		} catch ( ContainerExceptionInterface $e ) {
			// Log error but don't fail activation.
			$this->get_logger()->log_error( 'Failed to initialize order product type scan on activation.', __METHOD__ );
		}
	}

	/**
	 * Hooked to register_deactivation_hook() by an anonymous function in the plugin file.
	 *
	 * @return void
	 */
	public function deactivate(): void {
		if ( method_exists( Woocommerce_Analytics::class, 'maybe_remove_proxy_speed_module' ) ) {
	...
	/**
	 * Load the WooCommerce Analytics Tracking module.
	 *
	 * @return void
	 */
	public function load_woocommerce_analytics_tracking_module(): void {
	...
	/**
	 * Declare compatibility with WP Consent API.
	 *
	 * @return void
	 */
	private function declare_wp_consent_api_compatibility(): void {
		$plugin = plugin_basename( WC_ANALYTICS_FILE );
